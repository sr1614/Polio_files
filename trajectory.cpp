    ...
    if (num_groups > 1) {
        for (int i=0; i<num_groups; ++i) {
            for (int t=0; t<total_steps; t+=every) {
                double count = 0.0;
                if (sum_across) {
                    for (int j=0; j<min1(every, iteration-t); ++j) {
        ...
        for (int t=0; t<total_steps; t+=every) {
            double count = 0.0;
            if (sum_across) {
                for (int j=0; j<min1(every, total_steps-t); ++j) {
        ...
        for (int t=0; t<total_steps; t+=every) {
            double count = 0.0;
            if (sum_across) {
                for (int j=0; j<min1(every, num_time_steps-t); ++j) {
                    count += trajectory2[t+j];
                }
                count /= (double) min1(every, num_time_steps-t);
            }
            else {
                count = trajectory2[t];
            }
