...
void Model::simulate(std::vector<double> & model_params, std::vector<std::string> & param_names, Trajectory * traj, int start_dt, int end_dt, double step_size, int total_dt, gsl_rng * rng) {
    ...
    double R0_T0 = model_params[4];
    double R0_T1 = model_params[5];
    double R0_T2 = model_params[6];
    double R0_now = 0.0;
    double k = model_params[7];
    double rateE2I = model_params[8];
    double rateI2R = model_params[9];
    double Re = 0.0;
    ...
    if (start_dt < step_size) {  // Set initial number of infected
        ...
        int init_inf = (int)round(model_params[11]);
    ...
    for (int t=start_dt; t<end_dt; ++t) {
        ...
        if (recoveries > 100000) { // Assume that if more than 3000 people recover within a day, then the epidemic is too large.
            num_infected = 0.0;
        }
